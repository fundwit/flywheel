...
var _ = Describe("ProjectMembers", func() {
	...
	BeforeEach(func() {
		...
		Expect(testDatabase.DS.GormDB(context.Background()).AutoMigrate(&domain.Project{}, &domain.ProjectMember{}, &account.User{}).Error).To(BeNil())
	...
	Describe("CreateProjectMember", func() {
		It("should be able to create project member", func() {
			Expect(testDatabase.DS.GormDB(context.Background()).Save(&account.User{ID: 111, Name: "user111", Secret: "xxxx"}).Error).To(BeNil())
			...
			Expect(err).To(BeNil())
			Expect(testDatabase.DS.GormDB(context.Background()).Save(&account.User{ID: 222, Name: "test", Secret: "xxxx"}).Error).To(BeNil())
			...
			Expect(testDatabase.DS.GormDB(context.Background()).Save(&account.User{ID: 333, Name: "test333", Secret: "xxxx"}).Error).To(BeNil())
			Expect(namespace.CreateProjectMember(
			...
			q = []domain.ProjectMember{}
			Expect(testDatabase.DS.GormDB(context.Background()).Where(domain.ProjectMember{MemberId: sec.Identity.ID}).Find(&q).Error).To(BeNil())
			Expect(q).ToNot(BeNil())
		...
		It("non non-administrator nor non-project manager can create project member", func() {
			...
			g, err := namespace.CreateProject(&domain.ProjectCreating{Name: "demo", Identifier: "DEM"}, sec)
			Expect(err).To(BeNil())
			Expect(testDatabase.DS.GormDB(context.Background()).Save(&account.User{ID: 2, Name: "test", Secret: "xxxx"}).Error).To(BeNil())
	...
	Describe("DetailProjectMembers", func() {
		It("should return correct project member details", func() {
			s := testinfra.BuildSecCtx(types.ID(111), account.SystemAdminPermission.ID)
			details, err := namespace.DetailProjectMembers(nil, s)
			Expect(err).To(BeNil())
			Expect(len(*details)).To(BeZero())

			details, err = namespace.DetailProjectMembers(&[]domain.ProjectMember{}, s)
			Expect(err).To(BeNil())
			Expect(len(*details)).To(BeZero())

			namespace.QueryAccountNamesFunc = func(ids []types.ID, s *session.Session) (map[types.ID]string, error) {
			...
			namespace.QueryProjectNamesFunc = func(ids []types.ID, s *session.Session) (map[types.ID]string, error) {
				result := map[types.ID]string{}
				for _, id := range ids {
	...
	Describe("QueryProjectMemberDetails", func() {
		It("system admin can query members in all projects", func() {
			namespace.DetailProjectMembersFunc = func(pms *[]domain.ProjectMember, s *session.Session) (*[]domain.ProjectMemberDetail, error) {
