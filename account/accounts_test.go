...
var _ = Describe("userManage", func() {
	...
	BeforeEach(func() {
		...
		Expect(testDatabase.DS.GormDB().AutoMigrate(&account.User{}).Error).To(BeNil())
	})
	AfterEach(func() {
	...
	Describe("UpdateBasicAuthSecret", func() {
		It("should be able to update basic auth secret correctly", func() {
			...
			user := account.User{}
			Expect(testDatabase.DS.GormDB().Model(&account.User{}).Where(&account.User{ID: sec.Identity.ID}).First(&user).Error).To(BeNil())
	...
	Describe("CreateUser", func() {
		...
		It("should be able to create users correctly", func() {
			...
			user := account.User{}
			Expect(testDatabase.DS.GormDB().Model(&account.User{}).Where(&account.User{ID: u.ID}).First(&user).Error).To(BeNil())
	...
	Describe("QueryAccountNames", func() {
		It("should return correct account names", func() {
			ret, err := account.QueryAccountNames(nil)
			Expect(err).To(BeNil())
			Expect(len(ret)).To(BeZero())

			ret, err = account.QueryAccountNames([]types.ID{})
			Expect(err).To(BeNil())
			Expect(len(ret)).To(BeZero())
			...
			ret, err = account.QueryAccountNames([]types.ID{1, 2, 4})
			Expect(err).To(BeNil())
