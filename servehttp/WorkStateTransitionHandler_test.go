...
var _ = Describe("WorkStateTransitionHandler", func() {
	...
	BeforeEach(func() {
		router = gin.Default()
		workflowManager = &workflowManagerMock{}
		servehttp.RegisterWorkStateTransitionHer(router, workflowManager)
	})
	...
	Describe("handleCreate", func() {
		...
		It("should be able to handle service error", func() {
			workflowManager.CreateWorkStateTransitionFunc = func(c *flow.WorkStateTransitionBrief) (*flow.WorkStateTransition, error) {
		...
		It("should be able to create transition", func() {
			...
			Expect(err).To(BeNil())
			workflowManager.CreateWorkStateTransitionFunc = func(c *flow.WorkStateTransitionBrief) (*flow.WorkStateTransition, error) {
				return &flow.WorkStateTransition{ID: 123, CreateTime: t, WorkStateTransitionBrief: flow.WorkStateTransitionBrief{FlowID: 1, WorkID: 100, FromState: "PENDING", ToState: "DOING"}}, nil
			}
			...
			Expect(body).To(MatchJSON(`{"id": "123", "createTime": "` + timeString +
				`", "flowId":"1", "workId":"100", "fromState":"PENDING", "toState": "DOING"}`))
		})
	})
})
...
type workflowManagerMock struct {
	CreateWorkStateTransitionFunc func(*flow.WorkStateTransitionBrief) (*flow.WorkStateTransition, error)
}

func (m *workflowManagerMock) CreateWorkStateTransition(c *flow.WorkStateTransitionBrief) (*flow.WorkStateTransition, error) {
	return m.CreateWorkStateTransitionFunc(c)
}
