...
func TestCreateAvatar(t *testing.T) {
	...
	t.Run("Show be able to save avatar by self", func(t *testing.T) {
		store = ""
		err := CreateAvatar(123456, bytes.NewReader([]byte("hello world")), &security.Context{Identity: security.Identity{ID: 123456}})
	...
	t.Run("Show not be able to save avatar by other", func(t *testing.T) {
		store = ""
		err := CreateAvatar(123456, bytes.NewReader([]byte("hello world")), &security.Context{Identity: security.Identity{ID: 123}})
